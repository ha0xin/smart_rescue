...
StageNode::WorldCallback()
{
    ...
    for (size_t r = 0; r < this->robotmodels_.size(); ++r)
    {
        ...
        // broadcast ground truth pose transform
        tf::Quaternion gtQ;
        tf::quaternionMsgToTF(ground_truth_msg.pose.pose.orientation, gtQ);
        tf::Transform txGT(gtQ, tf::Point(ground_truth_msg.pose.pose.position.x, ground_truth_msg.pose.pose.position.y, 0.0));
        tf.sendTransform(tf::StampedTransform(txGT, sim_time,
                                              mapName("odom", r, static_cast<Stg::Model*>(robotmodel->positionmodel)),
                                              mapName("base_footprint", r, static_cast<Stg::Model*>(robotmodel->positionmodel))));
